...
description: "Wie AI Agents mit der echten Welt interagieren — und warum MCP der USB-C fÃ¼r AI-Integrationen geworden ist."
sidebar:
  order: 2
---
...
Das Problem: Bis 2024 musste jede AI-Applikation eigene Integrationen fÃ¼r jedes Tool bauen. 10 Apps und 10 Tools bedeuteten 100 Integrationen. Dann kam das Model Context Protocol — und veraenderte die Gleichung fundamental.

## Concept

### Wie Tool Use funktioniert

Ein LLM kann keine APIs aufrufen. Aber es kann strukturierte "Tool Calls" generieren — eine Kombination aus Funktionsname und Parametern. Die Host-Applikation fuehrt den Call aus und gibt das Ergebnis zurÃ¼ck. Das Modell reasoned dann Ã¼ber das Ergebnis.

Beispiel: Das Modell generiert `{"tool": "get_order_status", "params": {"order_id": "12345"}}`. Dein System fuehrt die Datenbankabfrage aus. Das Ergebnis geht zurÃ¼ck ans Modell, das daraus eine Kundenantwort formuliert.

### Was MCP ist

Das Model Context Protocol (MCP) ist ein offener Standard, angekuendigt von Anthropic im November 2024, fÃ¼r die Verbindung von AI-Assistenten mit externen Datenquellen, Tools und Services. Die Analogie: USB-C fÃ¼r AI-Integrationen — ein universelles Interface statt proprietÃ¤rer Adapter.

MCP definiert drei Primitives:
- **Tools** — Funktionen, die das Modell aufrufen kann (Code ausfÃ¼hren, API abfragen, E-Mail senden)
...
| Maerz 2025 | OpenAI adoptiert MCP fÃ¼r Agents SDK und ChatGPT |
...
Zu viele Tools erhÃ¶hen Latenz (Modell muss Ã¼ber Optionen nachdenken), Kosten (mehr Tokens) und Angriffsflaeche. Zu wenige Tools begrenzen die Faehigkeiten.
...
5. **FÃ¼r Fehler designen** — jeder Tool Call kann fehlschlagen; der Agent muss Errors graceful handlen

### Security-Implikationen

Tool Use eroeffnet neue Angriffsflaechen: Prompt Injection Ã¼ber Tool-Ergebnisse, uebermaessige Permissions, Data Exfiltration durch Agents, und Confused Deputy Attacks. Die Antwort: Principle of Least Privilege, Sandboxed Execution, Output Filtering und Audit Logging aller Tool Calls.
...
Du bist PM fÃ¼r ein Enterprise-CRM mit AI-Assistent. Das Team will dem Agenten folgende Tools geben:
...
**Write-Tools (6):** E-Mail senden, Deal-Status Ã¤ndern, Meeting einbuchen, Kontakt erstellen, Task zuweisen, Notiz hinzufuegen

Das sind 14 Tools fÃ¼r einen einzelnen Agent. Der erste Prototyp zeigt: Der Agent waehlt in 23% der Faelle das falsche Tool. Die durchschnittliche Antwortzeit ist 8 Sekunden — 4x langsamer als ohne Tool-Auswahl.

**Die Frage:** Wie strukturierst Du die Tool-Auswahl fÃ¼r den Launch?
...
**Warum:**
- 14 Tools ueberfordern das Modell — unter 15 ist die Empfehlung, aber unter 8 ist besser fÃ¼r Genauigkeit
