...
Euer Customer-Success-Team beschwert sich: Der AI-Assistent klingt "zu generisch". Er nutzt nicht die Fachsprache Eurer Branche, antwortet zu foermlich und formatiert Antworten anders als Euer Styleguide vorgibt. Der CTO schlaegt Fine-Tuning vor. Dein VP Engineering fragt: "Wieviel kostet das, und wann sehen wir Ergebnisse?"

Fine-Tuning ist der dritte Hebel in der AI-Optimierungshierarchie — nach Prompt Engineering und RAG. Es aendert die Gewichte des Modells, also sein internes Verhalten. Das ist maechtig, aber teuer und unflexibel. PMs muessen verstehen, wann Fine-Tuning gerechtfertigt ist und wann es eine teure Abkuerzung fuer besseres Prompting ist.

## Concept

### Was Fine-Tuning tatsaechlich tut

Fine-Tuning nimmt ein vortrainiertes LLM und trainiert es auf einem domainspezifischen Datensatz weiter. Es aendert die Gewichte — das interne Verhalten — statt nur Kontext zur Laufzeit bereitzustellen.

**Wofuer Fine-Tuning gut ist:**
- Konsistente Aenderung von Ton, Stil oder Format
...
- Tasks, deren Anforderungen sich haeufig aendern (Re-Training ist teuer)

### Fine-Tuning-Methoden

**LoRA (Low-Rank Adaptation):** Die dominante Methode 2025-2026. Friert die Originalgewichte ein und trainiert kleine Adapter-Matrizen. Erreicht 95% der Full-Fine-Tuning-Performance bei 10% der Kosten. Adapter sind klein (10-100 MB) und koennen getauscht werden — mehrere Spezialisierungen aus einem Basismodell. Kosten: 500-5.000 Euro pro Run.

**QLoRA:** Kombiniert LoRA mit Quantisierung (16-Bit auf 4-Bit). Ermoeglicht Fine-Tuning eines 7B-Modells auf einer Consumer-GPU (RTX 4090, ca. 1.500 Euro). Leichter Qualitaetsverlust vs. Standard-LoRA. Ideal fuer Experimente und Proof of Concept.

**Full Fine-Tuning:** Alle Parameter aktualisieren. Hoechste Qualitaet, aber prohibitiv teuer: 10.000-30.000+ Euro pro Run fuer 7B+ Modelle. Nur wenn Budget keine Rolle spielt.

**Managed Services (2026):** OpenAI bietet Fine-Tuning-API fuer GPT-4o und GPT-4o-mini. Anthropic fuer Claude (Enterprise Tier). Google Vertex AI fuer Gemini. Upload JSONL, zahle pro Training-Token — kein Infrastruktur-Management noetig.
...
**Die goldene Regel:** 500 von Experten kuratierte Beispiele schlagen 50.000 verrauschte. Der haeufigste Fehler bei Fine-Tuning ist nicht zu wenig Daten — sondern schlechte Daten.
...
| Braucht das Modell aktuelle/eigene Daten? | RAG hinzufuegen, dann pruefen | Weiter |
| Brauchst Du konsistente Verhaltensaenderung? | Weiter pruefen | Kein Fine-Tuning noetig |
| Hast Du 500+ hochwertige gelabelte Beispiele? | Weiter pruefen | Erst in Daten investieren |
| Verarbeitest Du 50.000+ Anfragen/Monat? | Weiter pruefen | ROI unwahrscheinlich |
| Kannst Du das Modell langfristig warten? | Fine-Tuning starten | Budget fuer Re-Training einplanen |
...
**Die Situation:**
- Zielgenauigkeit: 92%+ (Nutzer beschweren sich ueber falsche Kategorien)
...
**Warum:**
- **Hierarchie einhalten:** Prompt-Optimierung zuerst (1 Woche). Wenn 87% nicht reichen, hast Du die Baseline fuer Fine-Tuning
...
- **Option 3 ist ein Kostenproblem:** 18.000 statt 4.200 Euro/Monat fuer 6 Prozentpunkte Verbesserung. Fine-Tuning kostet einmalig 6.000 Euro und spart langfristig
...
- **Fine-Tuning aendert Verhalten, nicht Wissen.** Es macht das Modell nicht "schlauer" — es passt Stil, Format und domainspezifische Muster an. Fuer neues Wissen brauchst Du RAG.
...
- Datenqualitaet schlaegt Datenquantitaet. 500 Experten-Beispiele sind mehr wert als 50.000 verrauschte.
- Fine-Tuning ist kein einmaliger Aufwand. Modelle muessen neu trainiert werden, wenn sich Daten aendern oder Base Models aktualisiert werden. Budget fuer laufende Wartung einplanen.
