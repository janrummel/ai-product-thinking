...
Euer Customer-Success-Team beschwert sich: Der AI-Assistent klingt "zu generisch". Er nutzt nicht die Fachsprache Eurer Branche, antwortet zu foermlich und formatiert Antworten anders als Euer Styleguide vorgibt. Der CTO schlÃ¤gt Fine-Tuning vor. Dein VP Engineering fragt: "Wieviel kostet das, und wann sehen wir Ergebnisse?"

Fine-Tuning ist der dritte Hebel in der AI-Optimierungshierarchie — nach Prompt Engineering und RAG. Es Ã¤ndert die Gewichte des Modells, also sein internes Verhalten. Das ist maechtig, aber teuer und unflexibel. PMs mÃ¼ssen verstehen, wann Fine-Tuning gerechtfertigt ist und wann es eine teure Abkuerzung fÃ¼r besseres Prompting ist.

## Concept

### Was Fine-Tuning tatsaechlich tut

Fine-Tuning nimmt ein vortrainiertes LLM und trainiert es auf einem domainspezifischen Datensatz weiter. Es Ã¤ndert die Gewichte — das interne Verhalten — statt nur Kontext zur Laufzeit bereitzustellen.
...
- Tasks, deren Anforderungen sich hÃ¤ufig Ã¤ndern (Re-Training ist teuer)

### Fine-Tuning-Methoden

**LoRA (Low-Rank Adaptation):** Die dominante Methode seit 2025. Friert die Originalgewichte ein und trainiert kleine Adapter-Matrizen. Erreicht 95% der Full-Fine-Tuning-Performance bei 10% der Kosten. Adapter sind klein (10-100 MB) und kÃ¶nnen getauscht werden — mehrere Spezialisierungen aus einem Basismodell. Kosten: 500-5.000 Euro pro Run.

**QLoRA:** Kombiniert LoRA mit Quantisierung (16-Bit auf 4-Bit). Ermoeglicht Fine-Tuning eines 7B-Modells auf einer Consumer-GPU (RTX 4090, ca. 1.500 Euro). Leichter Qualitaetsverlust vs. Standard-LoRA. Ideal fÃ¼r Experimente und Proof of Concept.

**Full Fine-Tuning:** Alle Parameter aktualisieren. Hoechste QualitÃ¤t, aber prohibitiv teuer: 10.000-30.000+ Euro pro Run fÃ¼r 7B+ Modelle. Nur wenn Budget keine Rolle spielt.

**Managed Services (2026):** OpenAI bietet Fine-Tuning-API fÃ¼r GPT-4o und GPT-4o-mini. Anthropic fÃ¼r Claude (Enterprise Tier). Google Vertex AI fÃ¼r Gemini. Upload JSONL, zahle pro Training-Token — kein Infrastruktur-Management noetig.
...
| Braucht das Modell aktuelle/eigene Daten? | RAG hinzufuegen, dann prÃ¼fen | Weiter |
| Brauchst Du konsistente Verhaltensaenderung? | Weiter prÃ¼fen | Kein Fine-Tuning noetig |
| Hast Du 500+ hochwertige gelabelte Beispiele? | Weiter prÃ¼fen | Erst in Daten investieren |
| Verarbeitest Du 50.000+ Anfragen/Monat? | Weiter prÃ¼fen | ROI unwahrscheinlich |
| Kannst Du das Modell langfristig warten? | Fine-Tuning starten | Budget fÃ¼r Re-Training einplanen |
...
**Die Situation:**
- Zielgenauigkeit: 92%+ (Nutzer beschweren sich Ã¼ber falsche Kategorien)
...
**Warum:**
- **Hierarchie einhalten:** Prompt-Optimierung zuerst (1 Woche). Wenn 87% nicht reichen, hast Du die Baseline fÃ¼r Fine-Tuning
...
- **Option 3 ist ein Kostenproblem:** 18.000 statt 4.200 Euro/Monat fÃ¼r 6 Prozentpunkte Verbesserung. Fine-Tuning kostet einmalig 6.000 Euro und spart langfristig
...
- **Fine-Tuning Ã¤ndert Verhalten, nicht Wissen.** Es macht das Modell nicht "schlauer" — es passt Stil, Format und domainspezifische Muster an. FÃ¼r neues Wissen brauchst Du RAG.
...
- Datenqualitaet schlÃ¤gt Datenquantitaet. 500 Experten-Beispiele sind mehr wert als 50.000 verrauschte.
- Fine-Tuning ist kein einmaliger Aufwand. Modelle mÃ¼ssen neu trainiert werden, wenn sich Daten Ã¤ndern oder Base Models aktualisiert werden. Budget fÃ¼r laufende Wartung einplanen.
