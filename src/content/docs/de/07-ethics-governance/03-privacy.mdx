...
Ein Samsung-Ingenieur kopiert vertraulichen Quellcode in ChatGPT, um einen Bug zu finden. Innerhalb von Stunden wird der Fall intern eskaliert. Innerhalb von Tagen verbietet Samsung externe AI-Tools fÃ¼r alle Mitarbeiter.

Wenige Monate spÃ¤ter blockiert Italien ChatGPT wegen GDPR-Bedenken zur Trainingsdatenerhebung. Microsofts Recall-Feature — das alle paar Sekunden Screenshots macht — wird von Sicherheitsforschern zerlegt: Die Daten lagen im Klartext vor.
...
**2. Inference Data Privacy** — Was passiert mit User-Inputs und AI-Outputs? Werden Konversationen geloggt? Wer hat Zugriff? Werden sie fÃ¼r weiteres Training genutzt? Hier liegen die meisten PM-Entscheidungen.
...
### GDPR-Anforderungen fÃ¼r AI-Produkte

| Anforderung | Bedeutung fÃ¼r AI | PM-Implikation |
|-------------|-------------------|----------------|
...
| **Recht auf ErklÃ¤rung** | Art. 22: Kein rein automatisiertes Decision-Making mit signifikanter Wirkung | Human-in-the-Loop bei Entscheidungen, die Menschen betreffen |
| **Recht auf Loeschung** | User kÃ¶nnen Datenloesung verlangen — aber Modelle "vergessen" nicht | Technische LÃ¶sung fÃ¼r Machine Unlearning noetig |
| **Datenminimierung** | Nur erheben, was noetig ist | AI-Systeme, die alle verfÃ¼gbaren Daten absaugen, widersprechen diesem Prinzip |
...
| **Federated Learning** | Training auf dezentralen Daten; nur Modell-Updates verlassen das Geraet | Langsamere Konvergenz | Produktionsreif fÃ¼r bestimmte Use Cases |
...
| **Homomorphic Encryption** | Rechnen auf verschluesselten Daten ohne Entschluesselung | Extremer Overhead (1000x+ langsamer) | Forschungsphase fÃ¼r ML |

## Framework

Der **Privacy Tier Decision Guide** — waehle das richtige Privacy-Niveau fÃ¼r Dein Produkt:
...
| **Tier 4** | Shared API | Geringe Sensibilitaet; Ã¶ffentliche Daten | Standard-API-Zugriffe |
...
- Der API-Anbieter speichert Daten 30 Tage "fÃ¼r Qualitaetssicherung"
- Ein Krankenhaus-Kunde fordert eine DPIA vor dem Rollout
- Euer CTO sagt: "On-Device ist zu langsam und die QualitÃ¤t reicht nicht"

Die Kosten fÃ¼r eine Private-Cloud-LÃ¶sung (Tier 2) waeren ~3x hoeher als die aktuelle Cloud-API (Tier 4).
...
**Die beste Entscheidung:** Auf Tier 2 oder mindestens Tier 3 migrieren. Konkret: Einen API-Anbieter mit Zero-Retention-Policy waehlen oder eine Private-Cloud-LÃ¶sung aufbauen. Die DPIA durchfÃ¼hren — sie ist bei Gesundheitsdaten ohnehin Pflicht.

**Warum:**
- Patientendaten (PHI) Ã¼ber eine Shared API zu schicken, die 30 Tage speichert, ist ein GDPR/DSGVO-Verstoss, der auf eine Katastrophe wartet
...
- Anonymisierung allein reicht nicht — AI-Modelle kÃ¶nnen Individuen aus "anonymen" Daten re-identifizieren
- "Wir nutzen nur die API" heisst nicht, dass keine Daten das System verlassen — lies die Retention Policies des Anbieters
- Der Samsung-Vorfall hat Ã¼ber Nacht Enterprise-AI-Policies verÃ¤ndert; Privacy ist ein latentes Anliegen, das bei Vorfaellen akut wird
