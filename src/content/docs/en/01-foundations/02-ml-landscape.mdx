...
A neural network has input layers, hidden layers, and output layers. "Deep" means many hidden layers. Each connection has a weight learned during training. GPT-4 is rumored to have ~1.8 trillion such parameters.
...
- **RLHF** (Reinforcement Learning from Human Feedback): Pre-train → build a reward model from human feedback → fine-tune with PPO.
